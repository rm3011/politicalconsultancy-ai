...
const HeroTitle = memo(() => (
  ...
  >
    <h1 className="leading-[0.85] font-[family-name:var(--font-anton)] font-normal uppercase tracking-[-0.02em]">
      <span className="block text-[clamp(34px,9.5vw,56px)] xs:text-[clamp(38px,10vw,56px)] sm:text-[clamp(48px,12vw,72px)] md:text-[clamp(56px,14vw,88px)] lg:text-[clamp(68px,16vw,104px)] text-white">
        THE FUTURE
      </span>
      <br />
      <span className="block text-[clamp(34px,9.5vw,56px)] xs:text-[clamp(38px,10vw,56px)] sm:text-[clamp(48px,12vw,72px)] md:text-[clamp(56px,14vw,88px)] lg:text-[clamp(68px,16vw,104px)] text-white">
      ...
      <br />
      <span className="block text-[clamp(34px,9.5vw,56px)] xs:text-[clamp(38px,10vw,56px)] sm:text-[clamp(48px,12vw,72px)] md:text-[clamp(56px,14vw,88px)] lg:text-[clamp(68px,16vw,104px)] text-red-600 drop-shadow-[0_0_18px_rgba(220,38,38,.15)]">
        POLITICS.
      </span>
    </h1>
  </motion.div>
...
const ScrollIndicator = memo(() => (
  ...
  >
    ...
    <div className="flex items-center gap-3 md:gap-4 lg:gap-5">
      <div
        className="h-px w-12 sm:w-16 md:w-20 lg:w-32 bg-gradient-to-r from-red-600 to-transparent"
        aria-hidden="true"
      />
      <svg
    ...
    >
      <button
        ...
        className="group relative inline-flex items-center gap-2 sm:gap-2.5 md:gap-3
          ...
          hover:border-red-600/60 hover:bg-red-600/5
          hover:shadow-[0_0_60px_rgba(220,38,38,0.25)]
      ...
      >
        ...
        <span className="relative z-10 flex items-center gap-2 sm:gap-3 text-white/90 group-hover:text-white transition-colors duration-700">
          ...
          <ArrowRight className="w-3 h-3 xs:w-3.5 xs:h-3.5 sm:w-4 sm:h-4 md:w-4.5 md:h-4.5 transition-all duration-700 ease-[cubic-bezier(0.25,0.1,0.25,1)] group-hover:translate-x-2 group-hover:text-red-400 group-hover:scale-110" />
        </span>
      </button>
    </motion.div>
  );
...
// ============================================================
// IMAGE CONFIGURATIONS FOR EACH BREAKPOINT
// ============================================================
...
const getImageConfig = (width: number): ImageConfig => {
  // Mobile (320px - 767px): object-cover with center crop
  if (width < 480) {
    return { objectFit: 'cover', objectPosition: '60% 48%', scale: 1 };
  }
  if (width < 640) {
    return { objectFit: 'cover', objectPosition: '60% 48%', scale: 1 };
  }
  ...
  // Tablet (768px - 1024px): contain with slight zoom
  if (width < 1024) {
    return { objectFit: 'contain', objectPosition: '60% 50%', scale: 1.3 };
  }
  // Desktop (1024px+): full contain
...
const Hero = memo(({ className = '' }: HeroProps) => {
  ...
  // Update image config on resize
  useEffect(() => {
    const updateConfig = () => {
      const width = window.innerWidth;
      setImageConfig(getImageConfig(width));
    };
    updateConfig();
  ...
  return (
    ...
    >
      <h1 className="sr-only">
        THE EDGE WITH JOHN - The Future of New Gen Politics. AI Driven, Data Backed, Impact
        Focused.
      </h1>

      {/* Background image - Exact configurations per device */}
        ...
        {!imageError ? (
          <>
            <Image
              ...
              sizes="100vw"
              className={`
                transition-opacity duration-700 will-change-transform
              ...
              style={{
                ...
                filter:
                  'brightness(1.2) contrast(1.1) saturate(1.1) drop-shadow(0 0 30px rgba(220,38,38,0.1))',
              }}
      ...
      {/* Subtle red glow */}
      <div
        className="absolute z-10 pointer-events-none right-[5%] sm:right-[8%] md:right-[10%] top-[30%] sm:top-[25%]"
        aria-hidden="true"
      >
        <div
          className="w-[200px] h-[200px] sm:w-[280px] sm:h-[280px] md:w-[380px] md:h-[380px] rounded-full bg-red-600/15 blur-[150px] animate-pulse"
          style={{ animationDuration: '4s' }}
        />
      </div>

      {/* Content */}
      <div className="relative z-20 flex items-start pt-10 xs:pt-12 sm:pt-14 md:items-center md:pt-0 min-h-[calc(100vh-56px)] pb-12 sm:pb-16 md:pb-20">
        <div className="flex w-full max-w-[88%] xs:max-w-90 sm:max-w-100 md:max-w-125 lg:max-w-150 xl:max-w-160 flex-col justify-center px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12 2xl:px-16">
          {/* Badge */}
          <motion.div
          ...
          >
            <div
              className="h-px w-5 sm:w-6 md:w-8 bg-red-600 shadow-[0_0_20px_rgba(255,0,0,0.5)]"
              aria-hidden="true"
            />
